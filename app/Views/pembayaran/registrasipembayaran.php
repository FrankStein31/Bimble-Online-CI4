...
<div class="form-area">
    <div class="form-card">
        <div class="form-card-head">
            ...
            <p>Isi data berikut untuk mendaftar. Jadwal akan ditentukan oleh admin setelah pembayaran dikonfirmasi.</p>
        </div>
        <div class="form-card-body">
            <form action="<?= base_url('registrasi-pembayaran/submit') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="f-input" name="nama" value="<?= esc(session()->get('nama')) ?>" readonly>
                </div>

                <div class="form-group">
                    <div class="field-label">No. Telepon</div>
                    <input type="text" class="f-input" name="nomor_hp" value="<?= esc(session()->get('nomor_hp')) ?>" readonly>
                </div>

                <div class="form-group">
                    <div class="field-label">Pilih Program Bimbel <span style="color:#e53e3e;">*</span></div>
                    <select name="program_id" id="program_id" class="f-input" required>
                ...
                <div class="info-note">
                    📅 <strong>Jadwal & Pengajar</strong> akan ditetapkan oleh admin setelah pembayaran dikonfirmasi.<br>
                    Kamu akan mendapat 3 sesi bimbel per minggu sesuai program yang dipilih.
                </div>

                <?php if (!empty($rekening)): ?>
                <div class="bank-box">
                    <div class="bank-title">💳 Transfer Pembayaran ke:</div>
                ...
                <div class="btn-row">
                    <a href="<?= base_url('/registrasi-pembayaran/transfer-bank') ?>" class="btn-sec">🏦 Lihat Rekening</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const tagihanHidden  = document.getElementById('tagihan');
        ...
        programSelect.addEventListener('change', function() {
            ...
            if (harga) {
                tagihanHidden.value  = harga;
                tagihanDisplay.value = 'Rp ' + harga.toLocaleString('id-ID');
                tagihanDisplay.classList.add('filled');
            } else {
                tagihanHidden.value  = '';
                tagihanDisplay.value = '';
                tagihanDisplay.classList.remove('filled');
            }
        });
        ...
        fileInput.addEventListener('change', function() {
            if (fileInput.files.length > 0) {
                ...
                fileName.textContent = 'Klik atau seret untuk upload bukti transfer';
